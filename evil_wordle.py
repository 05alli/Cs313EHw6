...
class Keyboard:
    ...
    def __str__(self):
        ...
        """
        row_1 = color_word(self.rows[0], self.colors)
        row_2 = color_word(self.rows[1], self.colors)
        row_3 = color_word(self.rows[2], self.colors)

        keyboard_layout = f"{row_1}\n {row_2}\n  {row_3}"
...
class WordFamily:
    ...
    def __lt__(self, other):
        ...
        return str(self.feedback_colors) < str(other.feedback_colors)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    # Checking for correct letters and the correct position
    for i in range(NUM_LETTERS):
        if guessed_word_list[i] == secret_word_list[i]:
            feedback[i] = CORRECT_COLOR
            secret_word_list[i] = None

    # Checking for correct letters in the wrong position
    for i in range(NUM_LETTERS):
        if feedback[i] is None and guessed_word_list[i] in secret_word_list:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    # Grouping words by feedback pattern
    for word in remaining_secret_words:
        ...
        if feedback_tuple not in word_families:
            word_families[feedback_tuple] = []
        word_families[feedback_tuple].append(word)

    # Helper function to calculate the difficulty of the feedback
    def calculate_difficulty(feedback):
        difficulty = 0
        for color in feedback:
            difficulty += WordFamily.COLOR_DIFFICULTY.get(color, 0)
        return difficulty

    # Sorting families by 3 criteria
    sorted_families = sorted(word_families.items(), key=lambda x: (-len(x[1]), calculate_difficulty(x[0]), x[0]))

    hardest_family_feedback, hardest_family_words = sorted_families[0]
    feedback_colors = hardest_family_feedback

    return feedback_colors, hardest_family_words
