...
class Keyboard:
    ...
    def __str__(self):
        ...
        for row in self.rows:
            row_str = ""
        # Adjust spacing so that each row is aligned properly
        for letter in row:
            row_str += apply_color(self.colors[letter], letter) + " "
        result.append(row_str.rstrip().center(max_row_length * 2))  # Center-align rows
...
def fast_sort(lst):
    ...
    while left_idx < len(left_half) and right_idx < len(right_half):
        if left_half[left_idx] < right_half[right_idx]:
            sorted_result.append(left_half[left_idx])
            left_idx += 1
        else:
            sorted_result.append(right_half[right_idx])
            right_idx += 1

    sorted_result.extend(left_half[left_idx:])
    sorted_result.extend(right_half[right_idx:])
